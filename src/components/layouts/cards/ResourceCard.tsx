...
const ResourceCard = ({ data }: ResourceCardProps) => {
  ...
  const getDashboardPage = useDashboardPage((state) => state.getDashboardPage);
  ...
  const collectionList = data?.resourceCollections.map((id: number) => id + "");
        ...
        <div className="flex h-full flex-col p-5 xl:p-7">
          <div className="flex justify-start">
            <Image
              ...
              width={45}
              height={45}
              onError={() => setIcon("https://placehold.co/200x200")}
            />
          </div>
          <div className="mt-2">
          ...
          <div style={{ flex: 1 }} className="mt-4 flex flex-col justify-end">
            <ResourceTags tags={data.resourceTags} />
            <div className="mt-4">
              {getDashboardPage() === "shared" ||
                (getDashboardPage() === "public" && data?.sharedBy && (
