 ...
 * @template T - (Generic Type) The type of data to be sent in the request body.
...
export const register = async <T>(data: T): Promise<any> => {
	try {
		 ...
		 * @see errorApiResponseExtractor in '@/lib/utils' for more details
		 */
 ...
 * @param {T} data - The data to be sent in the request body.
 * @returns {Promise<any>} A Promise that resolves with the response data.
 * @throws {TErrorAPIResponse} Throws an error if login fails.
 *
 */
